...
docker-compose exec app bash -c "
    if [ ! -f 'vendor/autoload.php' ]; then
        composer create-project --prefer-dist laravel/laravel temp-laravel '10.*'
        cp -n temp-laravel/* . 2>/dev/null || :
        cp -r temp-laravel/app/* app/ 2>/dev/null || :
        cp -r temp-laravel/bootstrap . 2>/dev/null || :
        cp -r temp-laravel/config . 2>/dev/null || :
        cp -r temp-laravel/public . 2>/dev/null || :
        cp -r temp-laravel/resources . 2>/dev/null || :
        cp -r temp-laravel/routes . 2>/dev/null || :
        cp -r temp-laravel/storage . 2>/dev/null || :
        rm -rf temp-laravel
    fi
    composer install
